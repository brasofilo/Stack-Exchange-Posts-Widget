...
CREATE TABLE api_cache ( url                 TINYTEXT,
                     data                MEDIUMTEXT,
...
class SQLCache implements CacheBase
{
    ...
    /**
      ...
      *
      * Note: this function also makes sure that the table 'api_cache' exists,
    ...
    function __construct($type, $host='', $username=null, $password=null, $database=null)
    {
        try
        {
            ...
            // Check if we need to create the 'api_cache' table
    ...
    // Creates the 'api_cache' table if necessary
    private function CreateTableIfNeeded()
    {
        // Prepare a statement
        $statement = $this->database->prepare("SELECT * FROM api_cache LIMIT 0,1");
        ...
        if($statement === FALSE || $statement->execute() === FALSE)
        {
            ...
            if($statement === FALSE || $statement->execute() === FALSE)
                throw new APIException('Cache: An error occurred while creating the "api_cache" table.');
        }
    }
    ...
    {
        // Prepare a statement
        $statement = $this->database->prepare('SELECT data FROM api_cache WHERE url = ? AND retrieval_timestamp + ttl >= ?');
        if($statement === FALSE)
    ...
    public function AddToCache($url, $data, $ttl=null)
    {
        ...
        // Prepare a statement
        $statement = $this->database->prepare('INSERT INTO api_cache ( url, data, retrieval_timestamp, ttl ) VALUES (?,?,?,?)');
        if($statement === FALSE)
    ...
    {
        // Prepare a statement
        $statement = $this->database->query('DELETE FROM api_cache');
    ...
    {
        // Prepare a statement
        $statement = $this->database->prepare('DELETE FROM api_cache WHERE retrieval_timestamp + ttl < ?');
        if($statement === FALSE)
