...
class URL
{
    ...
    private $filter            = null;   // the default filter
    private $pagination_filter = null;   // the pagination filter to use
    
    // The amount of time to cache this URL for
    private $ttl;
    ...
    function __construct($domain_name=null)
    {
        ...
        // Set the filters and cache TTL for the URL
        $this->filter            = Filter::$default_filter;
        $this->pagination_filter = Filter::$default_pagination_filter;
        $this->ttl               = API::$cache_ttl;
    ...
    /**
      ...
      * \param $use_pagination_filter whether to use the pagination filter or not
      ...
      */
    public function CompleteURL($include_key=TRUE, $use_pagination_filter=FALSE)
    {
        // Determine the prefix we are going to use
        $prefix = ($this->prefix == self::SecureRequestPrefix)?'https://':'http://';
        
        // Begin constructing the URL
        $url = "{$prefix}api.stackexchange.com/" . API::GetAPIVersion();
        ...
        if($use_pagination_filter) $this->SetQueryStringParameter('filter', $this->pagination_filter);
        else                       $this->SetQueryStringParameter('filter', $this->filter);
    ...
    /**
      ...
      * \return the current instance
      *
      * This method accepts two filters. The first is used for requests that do not
      * require access to PagedResponse::Total(). The second (optional) filter will
      * be used when the total number of items is requested.
      */
    public function SetFilter($filter, $pagination_filter=null)
    {
        // Store the default filter
        if(is_string($filter)) $this->filter = $filter;
        else                   $this->filter = $filter->GetID();
        
        // Also set this filter as the paged filter
        $this->pagination_filter = $this->filter;
        
        // If a paged filter was specified, use it
        if(is_string($pagination_filter))     $this->pagination_filter = $pagination_filter;
        elseif(is_object($pagination_filter)) $this->pagination_filter = $pagination_filter->GetID();
    ...
    /**
      * \param $ttl the TTL for this URL
      * \return the current instance
      */
